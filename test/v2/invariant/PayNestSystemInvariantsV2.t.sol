...
contract PayNestSystemInvariantsV2 is Test {
    ...
    function invariant_SYS2_3_recipientPaymentDestinationConsistency() public view {
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            if (!registry.isUsernameAvailable(username)) {
                ...
                (bytes32[] memory scheduleIds, IPayments.Schedule[] memory schedules) =
                    plugin.getUserSchedules(username);
    ...
    function invariant_SYS2_4_systemStateConsistencyUnderUpdates() public view {
        for (uint256 i = 0; i < controllers.length; i++) {
            ...
            if (!registry.isUsernameAvailable(username)) {
                ...
                try plugin.getUserSchedules(username) returns (
                    bytes32[] memory scheduleIds, IPayments.Schedule[] memory
                ) {
                    // Should not revert
    ...
    function registerUsernameWithSeparation(uint256 controllerSeed, uint256 recipientSeed, uint256 usernameSeed)
        public
        useController(controllerSeed)
    {
    ...
    function updateUsernameRecipient(uint256 controllerSeed, uint256 newRecipientSeed, uint256 usernameSeed)
        public
        useController(controllerSeed)
    {
    ...
    function createPaymentForUsername(uint256 managerSeed, uint256 usernameSeed, uint256 amountSeed, bool isStream)
        public
        useManager(managerSeed)
    {
        string memory username = string(abi.encodePacked("sep", vm.toString(bound(usernameSeed, 0, 999))));
    ...
    function transferUsernameControl(uint256 controllerSeed, uint256 newControllerSeed, uint256 usernameSeed)
        public
        useController(controllerSeed)
    {
